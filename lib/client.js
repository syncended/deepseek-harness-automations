window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function workspaceNavigationRowFromClick(target, region) {
      ...
      if (!row || (region && !region.contains?.(row))) return null;
      const nestedButton = target.closest?.("button") || null;
      return nestedButton && nestedButton !== row ? null : row;
    }

    function AutomationsSidebarAction({ wide, disclosure, sessionsRuntime, panelNavigation, usePanelInfo }) {
      const legacyOpen = useSyncExternalStore(
        disclosure.subscribe,
        disclosure.getSnapshot,
        disclosure.getSnapshot,
      );
      // Registration chooses one API for this component's entire lifetime.
      // On current hosts, layout selection is the source of truth (including
      // navigation to another plugin or back to the current session).
      const open = panelNavigation
      ...
      useEffect(() => {
        if (panelNavigation) return undefined;
        const onWorkspaceClick = (event) => {
          if (!disclosure.getSnapshot()) return;
          if (!workspaceNavigationRowFromClick(event.target)) return;
          restoreTriggerFocusRef.current = false;
          disclosure.close();
        };
        // Bubble after DSH's row handler so it can open the selected session
        // before this higher-priority center workspace is unmounted.
        document.addEventListener("click", onWorkspaceClick);
        return () => document.removeEventListener("click", onWorkspaceClick);
      }, [disclosure, panelNavigation]);

      useEffect(() => {
        if (panelNavigation || !open) return undefined;
        const initialCurrent = sessionsRuntime.list.getSnapshot().current;
        return sessionsRuntime.list.subscribe(() => {
          const current = sessionsRuntime.list.getSnapshot().current;
          if (current === initialCurrent) return;
          restoreTriggerFocusRef.current = false;
          disclosure.close();
        });
      }, [disclosure, open, sessionsRuntime, panelNavigation]);

      useEffect(() => {
        const previous = previousOpenRef.current;
        previousOpenRef.current = open;
        if (panelNavigation || !previous || open) return undefined;
    ...
    function apply(ctx) {
      const panelNavigation = typeof ctx.layout?.selectPanel === "function";
      const disclosure = createDisclosureStore();
      if (panelNavigation) {
        disclosure.open = () => ctx.layout.selectPanel(PANEL_ID);
        disclosure.close = () => ctx.layout.selectPanel(null);
      }
      ...
      if (panelNavigation) {
        ...
        // Older hosts expose a priority-based conversation slot instead.
        ctx.effect(
      ...
      ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
        ...
        order: 50,
        label: "Automations",
        inject: () => ({ disclosure, sessionsRuntime: ctx.sessions, panelNavigation }),
